# Project Index

Start here for durable MiniPainterHub project knowledge.

## Project

- [Project README](<../10 Project/README.md>)
- [Project structure](<../10 Project/project_structure.txt>)

## Vault Navigation

- [Agent navigation](<Agent Navigation.md>)
- [Knowledge map](<Knowledge Map.md>)
- [Vault specification](<Vault Specification.md>)

## Engineering

- [Engineering index](<../20 Engineering/README.md>)
- [Architecture](<../20 Engineering/ARCHITECTURE.md>)
- [Blazor WASM performance log](<../20 Engineering/Blazor WASM Performance Log.md>)
- [Code style](<../20 Engineering/CODE_STYLE.md>)
- [Deployment](<../20 Engineering/DEPLOYMENT.md>)
- [.NET 10 upgrade](<../20 Engineering/DOTNET_10_UPGRADE.md>)

## Process

- [Process index](<../30 Process/README.md>)
- [Contributing](<../30 Process/CONTRIBUTING.md>)
- [Best practices](<../30 Process/BEST_PRACTICES.md>)
- [Anti-patterns](<../30 Process/ANTI_PATTERNS.md>)
- [AI workflow playbook](<../30 Process/WORKFLOW_PLAYBOOK.md>)
- [UI quality playbook](<../30 Process/UI_QUALITY_PLAYBOOK.md>)
- [Decisions](<../30 Process/Decisions/README.md>)

## Design And Visuals

- [Design index](<../40 Design/README.md>)
- [Workshop gallery mockup](<../40 Design/Workshop Gallery/>)
- [Visual assets index](<../50 Visual Assets/README.md>)
- [Portfolio screenshots](<../50 Visual Assets/Portfolio Screenshots/>)

## Vault Operations

- [ADR template](<../_templates/adr-template.md>)
- [Vault change logs](<../_logs/vault-changes/>)
- [Reflection logs](<../_logs/reflections/>)
- [Archive](<../_archive/>)

## Root Pointer Files

- [Agent entry point](../../AGENT.md)
- [Context control](../../AGENTS.md)
