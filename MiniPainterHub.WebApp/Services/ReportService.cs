...
namespace MiniPainterHub.WebApp.Services
{
    public sealed class ReportService : IReportService
    {
        ...
        public Task<bool> ReportPostAsync(int postId, CreateReportRequestDto request) =>
            _api.SendAsync(new HttpRequestMessage(HttpMethod.Post, $"api/reports/posts/{postId}") { Content = ApiClient.CreateJsonContent(request) });

        public Task<bool> ReportCommentAsync(int commentId, CreateReportRequestDto request) =>
            _api.SendAsync(new HttpRequestMessage(HttpMethod.Post, $"api/reports/comments/{commentId}") { Content = ApiClient.CreateJsonContent(request) });
        ...
        public Task<bool> ResolveAsync(long reportId, ResolveReportRequestDto request) =>
            _api.SendAsync(new HttpRequestMessage(HttpMethod.Post, $"api/reports/{reportId}/resolve") { Content = ApiClient.CreateJsonContent(request) });
