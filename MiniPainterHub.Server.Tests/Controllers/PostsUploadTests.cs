...
public class PostsUploadTests : IClassFixture<PostsUploadTests.TestApplicationFactory>
{
    ...
    public class TestImageStore : IImageStore
    {
        ...
        public Task<ImageStoreResult> SaveAsync(int postId, Guid imageId, ImageVariants variants, System.Threading.CancellationToken ct)
        {
            lock (_saved)
            {
            ...
            var baseUrl = $"https://test.local/images/{postId}/";
        ...
        private void SaveVariant(int postId, Guid imageId, string suffix, ImageVariant variant)
        {
            _saved.Add(new StoredVariant($"{postId}_{imageId:D}_{suffix}.{variant.Extension}", variant));
        }
