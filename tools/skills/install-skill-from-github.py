#!/usr/bin/env python3
"""Install a skill from a GitHub repo into global/repo/both locations."""
...
def _default_repo_root() -> str:
    # tools/skills/install-skill-from-github.py -> repo root is 2 levels up.
...
def _request(url: str) -> bytes:
    return github_request(url, "minipainterhub-skill-install")
...
def _parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(
        description="Install a skill from GitHub into global/repo/both locations."
    )
    ...
    parser.add_argument(
        ...
        default="repo",
        help="Install destination: ~/.codex/skills, ./.agents/skills, or both",
    )
    parser.add_argument(
        "--repo-root",
        help="Path to repository root (defaults to inferred root from this script location)",
    )
    ...
    try:
        ...
        for path in source.paths:
            ...
            for root in roots:
                ...
                planned.append((path, skill_name, dest_dir))

        tmp_dir = tempfile.mkdtemp(prefix="skill-install-", dir=_tmp_root())
        try:
            repo_checkout = _prepare_repo(source, args.method, tmp_dir)
            for path, _, _ in planned:
                _validate_skill(os.path.join(repo_checkout, path))
            for path, _, dest_dir in planned:
                skill_src = os.path.join(repo_checkout, path)
                _copy_skill(skill_src, dest_dir)
        finally:
            if os.path.isdir(tmp_dir):
                shutil.rmtree(tmp_dir, ignore_errors=True)

        for _, skill_name, dest_dir in planned:
